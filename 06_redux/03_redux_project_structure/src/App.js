 ...
 * redux-actions : redux의 reducer를 정의하기 위해서 사용하니 lib
 * redux-thunk : 비동기 작업을 처리할 때 사용하는 미들웨어
 * redux-logger : 미들웨어에서 state의 상태 변화 및 action을 로그로 남겨 상태를 볼 수 있도록함
 * redux-devtools-extension : 크롬의 별도 extension으로 설치되어 리덕스가 가직 있는 장점 중 하나인
 * 타임스냅샷(state의 변화 내용을 기록함)을 이용하여 이전의 상태로 돌아가거나 현재의 상태를 표시하는 줄
*/
